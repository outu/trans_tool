...
namespace ClientApp\Foundations\Bootstrap;
...
class CoreServiceProvider
{
    ...
    public function bindConfig()
    {
        $this->m_app->singleton(
            'config',
            'ClientApp\Foundations\Repository\BaseConfig'
        );
    }
    ...
    protected function bindLogger()
    {
        ...
        $logHandlerFile = new StreamHandler(RUNTIME_PATH.'ClientApp.log');
        $logFormatter = new LineFormatter();
        $logHandlerStdout->setFormatter($logFormatter);
        $logHandlerFile->setFormatter($logFormatter);
        $logger = new Logger('client', [$logHandlerStdout, $logHandlerFile]);
    ...
    {
        $sControllerNameSpace = 'CapsheafBuilder\Modules\\';
