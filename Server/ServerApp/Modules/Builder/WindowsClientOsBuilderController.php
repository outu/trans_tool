...
class WindowsClientOsBuilderController extends BaseController
{
    ...
    public function build($sVersion = '', $sBuildToolPath = "", $bEncrypt = false, $sRelease = 'alpha')
    {
        try {
            ...
            $sBuildExeFile_x86 = "{$sRootDir}/Builds/Capsheaf-Agent-OS-{$sVersion}-{$sDateToday}-{$sGitVersion}-{$sRelease}-x86.exe";
            $sBuildExeFile_x64 = "{$sRootDir}/Builds/Capsheaf-Agent-OS-{$sVersion}-{$sDateToday}-{$sGitVersion}-{$sRelease}-x64.exe";
            $sBuildTempFile_x86 = "{$sRootDir}/Components/CapsheafPkg_Windows_Os/Media/OSController_x86/Package/OSController_x86.exe";
            $sBuildTempFile_x64 = "{$sRootDir}/Components/CapsheafPkg_Windows_Os/Media/OSController_x64/Package/OSController_x64.exe";
            $nTempExeMTime='';
            if(file_exists($sBuildTempFile_x86)) {
                $nTempExeMTime = FileSystem::lastModified($sBuildTempFile_x86);//Time OR False
            }
            $sIsmProjectFile = "{$sRootDir}/Components/CapsheafPkg_Windows_Os/Nisec.ism";
            $sCommand_x86 = "\"{$sBuildToolPath}\" -p \"{$sIsmProjectFile}\" -r OSController_x86 -c COMP";
            $sCommand_x64 = "\"{$sBuildToolPath}\" -p \"{$sIsmProjectFile}\" -r OSController_x64 -c COMP";

            $sConfigClientFile_x86 = "{$sRootDir}/Components/CapsheafPkg_Windows_Os/OSController_x86/version";
            $sConfigClientFile_x64 = "{$sRootDir}/Components/CapsheafPkg_Windows_Os/OSController_x64/version";
