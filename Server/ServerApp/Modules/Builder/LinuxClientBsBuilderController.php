...
class LinuxClientBsBuilderController extends BaseController
{
    ...
    public function build($sVersion = '', $bEncrypt = false, $sRelease = 'alpha')
    {
        try {
            ...
            $sBuildZipFile = "{$sRootDir}/Builds/Capsheaf-Agent-BS"."-{$sVersion}-{$sDateToday}-{$sGitVersion}-{$sRelease}-x86_64.zip";

            $zip = new Zip($bEncrypt);
            $nResult = $zip->open($sBuildZipFile, ZipArchive::CREATE|ZipArchive::OVERWRITE);
            if ($nResult === true) {

                $sConfigClientFile = "{$sRootDir}/Components/CapsheafPkg_Linux_Bs/CapsheafAgent-BS/version";
                file_put_contents($sConfigClientFile, "{$sVersion} : {$sGitVersion}");

                $zip->disableEncrypt();
                $zip->addFolder("{$sRootDir}/Components/CapsheafPkg_Linux_Bs/",
                    "{$sRootDir}/Components/CapsheafPkg_Linux_Bs/",
                    '',
                    array("{$sRootDir}/Components/CapsheafPkg_Linux_Bs/.gitignore"));
                $zip->close();
